function [Archive, LBA, LBA_SCD, PBA, PBA_SCD, idx, maxCluster] = Environmental_Selection_APC_small(Problem, Archive, Population, LBA, PBA, n_PBA)
   %% construct Archive by APC
    ...
    % Apply APC clustering on decision variables
    temp_Archive = Archive;
    [idx, maxCluster] = APC(temp_Archive.decs);
    temp_Archive_nd = [];
    temp_idx_d_nd = [];
    for i = 1:maxCluster
